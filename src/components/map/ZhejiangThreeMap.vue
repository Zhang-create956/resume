...
import shenzhenTerrainData from '../../assets/maps/shenzhen-terrain.json';
import { initialMapState, loadMapLevel, prefetchMapLevel, type MapScope, type MapState } from './mapDataAdapter';
import { loadStreetDetail, type StreetDetailData } from './mapDetailDataAdapter';
...
const featureHighlightMaterials = new Map<string, THREE.MeshStandardMaterial[]>();
...
const shenzhenTerrainSceneScale = 22;
type ShenzhenTerrainGrid = {
...
const shenzhenTerrainGrid = shenzhenTerrainData as unknown as ShenzhenTerrainGrid;
...
function usesShenzhenDemTerrain() {
  return (currentState.scope === 'city' && currentState.code === '440300')
    || (currentState.scope === 'district' || currentState.scope === 'detail')
      && currentState.code.startsWith('4403');
}

function sampleShenzhenTerrainMeters(coord: readonly [number, number]) {
  const [west, south, east, north] = shenzhenTerrainGrid.bbox;
  const [longitude, latitude] = coord;
  const u = THREE.MathUtils.clamp((longitude - west) / Math.max(0.000001, east - west), 0, 1);
  // The DEM file is stored north-to-south, matching the source Terrarium tiles.
  const v = THREE.MathUtils.clamp((north - latitude) / Math.max(0.000001, north - south), 0, 1);
  const x = u * (shenzhenTerrainGrid.width - 1);
  const y = v * (shenzhenTerrainGrid.height - 1);
  const left = Math.floor(x);
  const top = Math.floor(y);
  const right = Math.min(shenzhenTerrainGrid.width - 1, left + 1);
  const bottom = Math.min(shenzhenTerrainGrid.height - 1, top + 1);
  const fractionX = x - left;
  const fractionY = y - top;
  const read = (column: number, row: number) => (
    shenzhenTerrainGrid.elevations[row * shenzhenTerrainGrid.width + column] ?? 0
  );
  const northValue = THREE.MathUtils.lerp(read(left, top), read(right, top), fractionX);
  const southValue = THREE.MathUtils.lerp(read(left, bottom), read(right, bottom), fractionX);
  return THREE.MathUtils.lerp(northValue, southValue, fractionY);
}

function getTerrainElevationOffset(coord: readonly [number, number]) {
  if (!usesShenzhenDemTerrain()) return 0;
  const elevation = sampleShenzhenTerrainMeters(coord);
  const normalized = THREE.MathUtils.clamp(
    (elevation - shenzhenTerrainGrid.baselineMeters) / shenzhenTerrainGrid.rangeMeters,
    0,
    1,
  );
  return normalized * shenzhenTerrainSceneScale;
}

function getTerrainSurfaceZ(coord: readonly [number, number], z: number) {
  return z + getTerrainElevationOffset(coord);
}

function unprojectMapCoord(x: number, y: number): Position {
  return [
    lonMin + (x - projectedOffsetX) / mapScale,
    latMax - (y - projectedOffsetY) / mapScale,
  ];
}

function projectMapPixel(x: number, y: number, z: number) {
  return new THREE.Vector3(
    x - mapWidth / 2,
    -(y - mapHeight / 2),
    getTerrainSurfaceZ(unprojectMapCoord(x, y), z),
  );
}
...
function createTopTerrainGeometry(shapes: THREE.Shape[]) {
  let geometry: THREE.BufferGeometry = new THREE.ShapeGeometry(shapes);
  if (usesShenzhenDemTerrain()) {
    const sourceGeometry = geometry;
    geometry = new TessellateModifier(11, 7).modify(sourceGeometry);
    sourceGeometry.dispose();
  }
  applyMapTerrainUv(geometry);
  if (usesShenzhenDemTerrain()) {
    const positions = geometry.getAttribute('position') as THREE.BufferAttribute;
    for (let index = 0; index < positions.count; index += 1) {
      const coord = unprojectMapCoord(
        positions.getX(index) + mapWidth / 2,
        mapHeight / 2 - positions.getY(index),
      );
      positions.setZ(index, getTerrainElevationOffset(coord));
    }
...
function makeBoundary(ring: Position[], z: number, material: THREE.Material) {
  const points = ring.map((coord) => projectPoint(coord, getTerrainSurfaceZ(coord, z)));
  const first = points[0];
  const last = points[points.length - 1];
...
function makeBoundarySegments(rings: Position[][], z: number, material: THREE.Material) {
  ...
  rings.forEach((ring) => {
    for (let index = 0; index < ring.length; index += 1) {
      points.push(
        projectPoint(ring[index], getTerrainSurfaceZ(ring[index], z)),
        projectPoint(ring[(index + 1) % ring.length], getTerrainSurfaceZ(ring[(index + 1) % ring.length], z)),
      );
    }
  });
  const geometry = new THREE.BufferGeometry().setFromPoints(points);
...
function createProvinceSilhouetteLoop() {
  ...
  return largestLoop.map((point) => {
    ...
    return projectMapPixel(x, y, provinceChaseZ);
  });
}
...
function createProvinceChaseLight() {
  ...
  if (provinceSilhouetteLoop.length > 4) {
    ...
    fallbackLoop?.forEach((coord, index) => {
      ...
      appendSegment(
        projectPoint(coord, getTerrainSurfaceZ(coord, provinceChaseZ)),
        projectPoint(nextCoord, getTerrainSurfaceZ(nextCoord, provinceChaseZ)),
      );
    });
  }
...
function createProvinceOutlineSegments(z: number, material: THREE.Material, renderOrder: number) {
  ...
  getProvinceOuterEdges().forEach((edge) => {
    points.push(
      projectPoint(edge.start, getTerrainSurfaceZ(edge.start, z)),
      projectPoint(edge.end, getTerrainSurfaceZ(edge.end, z)),
    );
  });
  const geometry = new THREE.BufferGeometry().setFromPoints(points);
...
function getEffectSourceLabelName(labels = currentLabels) {
  ...
  if (currentState.scope === 'city') {
    ...
    if (currentState.code === '440300') {
      return labels.find((label) => label.name === '南山区')?.name ?? labels[0]?.name;
    }
...
function createFlyLines() {
  ...
  const sourcePoint = projectPoint(sourceLabel.coord, getTerrainSurfaceZ(sourceLabel.coord, lineZ));
  ...
  flyLineTargets.forEach((targetLabel, index) => {
    const targetPoint = projectPoint(targetLabel.coord, getTerrainSurfaceZ(targetLabel.coord, lineZ));
...
function createProvinceSideWalls(material: THREE.Material) {
  ...
  getProvinceOuterEdges().forEach((edge) => {
    const topStart = projectPoint(edge.start, getTerrainSurfaceZ(edge.start, topZ));
    const topEnd = projectPoint(edge.end, getTerrainSurfaceZ(edge.end, topZ));
...
function createPolygonSideWalls(polygon: Position[][], material: THREE.Material) {
  ...
  polygon.forEach((ring) => {
    for (let index = 0; index < ring.length; index += 1) {
      const start = ring[index];
      const end = ring[(index + 1) % ring.length];
      const topStart = projectPoint(start, getTerrainSurfaceZ(start, topZ));
      const topEnd = projectPoint(end, getTerrainSurfaceZ(end, topZ));
...
function projectDetailPoint(coord: Position, z: number) {
  return projectPoint(coord, getTerrainSurfaceZ(coord, z));
}

function createRoadDetailLayer(detail: StreetDetailData) {
  const group = new THREE.Group();
  const pathsByKind = new Map<string, THREE.Vector3[]>();
  const majorRoadKinds = new Set(['motorway', 'trunk', 'primary', 'secondary']);

  detail.roads.forEach((road) => {
    const kind = majorRoadKinds.has(road.kind) ? 'major' : 'local';
    const segments = pathsByKind.get(kind) ?? [];
    for (let index = 1; index < road.coordinates.length; index += 1) {
      ...
      segments.push(projectDetailPoint(start, 50.6), projectDetailPoint(end, 50.6));
    }
    pathsByKind.set(kind, segments);
  });

  pathsByKind.forEach((points, kind) => {
    if (!points.length) return;
    const material = new THREE.LineBasicMaterial({
      color: kind === 'major' ? mapTheme.accent : mapTheme.accentSoft,
      transparent: true,
      opacity: kind === 'major' ? 0.9 : 0.52,
      blending: THREE.AdditiveBlending,
      depthTest: true,
      depthWrite: false,
    });
    const roadLines = new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(points), material);
    roadLines.renderOrder = 13;
    group.add(roadLines);
  });
...
function makeBuildingGeometry(building: StreetDetailData['buildings'][number]) {
  const shape = makeShape([building.coordinates]);
  const height = THREE.MathUtils.clamp(building.heightMeters * 0.105, 1.2, 23);
  ...
  const position = geometry.getAttribute('position') as THREE.BufferAttribute;
  for (let index = 0; index < position.count; index += 1) {
    const mapCoord = unprojectMapCoord(
      position.getX(index) + mapWidth / 2,
      mapHeight / 2 - position.getY(index),
    );
    position.setZ(index, position.getZ(index) + getTerrainSurfaceZ(mapCoord, 49));
  }
...
function createBuildingDetailLayer(detail: StreetDetailData) {
  const group = new THREE.Group();
  const geometries: THREE.BufferGeometry[] = [];
  detail.buildings.forEach((building) => {
    try {
      geometries.push(makeBuildingGeometry(building));
    } catch {
      // A malformed public footprint should not prevent the rest of a street
      // from rendering; valid OSM ways remain in the same on-demand response.
    }
  });
  ...
  buildingEdges.renderOrder = 14;
  group.add(buildingEdges);
  return group;
}

function createStreetDetailLayers(detail: StreetDetailData) {
  const group = new THREE.Group();
  group.name = 'openstreetmap-street-detail';
  group.userData.source = detail.source;
  group.userData.sourceUrl = detail.sourceUrl;
  group.add(createRoadDetailLayer(detail));
  group.add(createBuildingDetailLayer(detail));
  return group;
}
...
async function createMap() {
  ...
  const topMaterial = createMapTerrainMaterial({
    elevationScale: usesShenzhenDemTerrain()
      ? shenzhenTerrainSceneScale
      : currentState.scope === 'country' ? 5.6 : currentState.scope === 'province' ? 8.2 : 7.2,
    normalStrength: currentState.scope === 'country' ? 0.82 : 1,
    roughness: 0.94,
    textureOpacity: currentState.scope === 'country' ? 0.84 : 0.9,
    heightSource: usesShenzhenDemTerrain() ? 'shenzhen-dem' : 'stylized',
  });
  topMaterial.alphaTest = 0.02;
  if (usesShenzhenDemTerrain()) {
    // Shenzhen's DEM is baked into the tessellated mesh positions so normals,
    // lighting, raycasting, outlines, and side walls share one true surface.
    topMaterial.displacementMap = null;
  ...
  for (let featureIndex = 0; featureIndex < renderableFeatures.length; featureIndex += 1) {
    ...
    const topGeometry = createTopTerrainGeometry(shapes);
    ...
    const topGlow = new THREE.Mesh(createTopTerrainGeometry(shapes), topGlowMaterial);
    ...
    const highlightMesh = new THREE.Mesh(createTopTerrainGeometry(shapes), highlightInstanceMaterial);
  ...
  if (currentState.scope === 'detail' && currentState.detailData) {
    await waitForBuildSlice();
    group.add(createStreetDetailLayers(currentState.detailData));
  }
...
function createCityMarkers(group: THREE.Group) {
  currentLabels.forEach((city) => {
    const anchorPoint = projectPoint(city.coord, getTerrainSurfaceZ(city.coord, 58));
...
function refreshDrillControl() {
  ...
  // Returning is also the escape hatch for a slow public-data request, so it
  // must stay available while a deeper level is being prepared.
  const backDisabled = !drillStack.length ? 'disabled' : '';
...
async function drillToFeature(featureName: string) {
  ...
  const operationId = ++drillOperationId;
  isDrilling = true;
  refreshDrillControl();
  try {
    const detailData = target.scope === 'detail'
      ? await loadStreetDetail(target.code)
      : undefined;
    if (operationId !== drillOperationId) return;
    const nextGeoJson = detailData?.streetGeoData
      ?? await loadMapLevel(target.scope, target.code);
    if (operationId !== drillOperationId) return;
    ...
    if (operationId === drillOperationId) console.warn('Map drilldown failed', error);
  } finally {
    if (operationId === drillOperationId) {
      isDrilling = false;
      refreshDrillControl();
    }
  }
...
async function drillBack() {
  // Invalidate an unresolved down-drill before restoring the previous state.
  // This makes the control responsive even when a public OSM mirror is slow.
  if (isDrilling) {
    drillOperationId += 1;
    isDrilling = false;
  }
